  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-xs p-4 overflow-y-auto font-sans">
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-emerald-100">
        ...
        {/* Header */}
        <div className="sticky top-0 z-10 bg-[#065f46] text-white px-6 py-4 flex items-center justify-between border-b border-emerald-700">
          <div>
            <h3 className="text-lg font-extrabold tracking-tight">Report Civic Complaint</h3>
        ...
        <div className="p-6">
          {/* Map Pin Status Bar */}
          <div className={`mb-4 p-3 rounded-lg flex items-center justify-between text-xs font-semibold ${
            ...
            <div className="flex items-center space-x-2">
              <MapPin className={`w-4 h-4 ${pinLocation ? 'text-emerald-600' : 'text-amber-600'}`} />
              <span>
                {pinLocation 
            ...
            {!pinLocation && (
              <span className="px-2 py-0.5 bg-amber-600 text-white rounded text-[10px] uppercase font-bold">Pin Missing</span>
            )}
          </div>

          {/* Duplicate Alert Box */}
          {duplicateAlert && (
            <div className="mb-4 p-4 bg-amber-50 border-2 border-amber-300 rounded-xl text-amber-950 space-y-2">
              ...
              {duplicateAlert.existing_complaint_id && (
                <div className="pt-2 flex items-center justify-between border-t border-amber-200/60">
                  ...
                  <button
                    ...
                    className="px-3.5 py-1.5 bg-emerald-700 hover:bg-emerald-800 text-white font-bold text-xs rounded-lg shadow-sm flex items-center space-x-1"
                  >
          ...
          {error && !duplicateAlert && (
            <div className="mb-4 p-3 bg-red-50 text-red-700 text-xs rounded-lg border border-red-200 flex items-center space-x-2">
          ...
          {step === 1 ? (
            ...
            <form onSubmit={handlePreCheck} className="space-y-5">
              <div>
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs text-slate-600 space-y-2">
                ...
                <p>Before submitting a new report, our system checks if this issue has already been reported within a 200m radius in your ward.</p>
              </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                ...
                <div className="flex space-x-2">
                  <button
                    type="button"
                    onClick={onClose}
                    className="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 text-xs font-bold rounded-lg"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={loading}
                    className="px-5 py-2 bg-[#065f46] hover:bg-emerald-800 text-white text-xs font-bold rounded-lg shadow-md transition"
                  >
