...
import { CheckCircle2, XCircle, AlertTriangle, Send, RefreshCw, Eye } from "lucide-react";
...
export default function CitizenVerificationSimulator({ complaint, onVerify }) {
  ...
  return (
    <div className="bg-purple-50/80 border-2 border-purple-200 rounded-xl p-4 my-4">
      ...
      {!showRejectionForm ? (
        ...
        <form onSubmit={handleReject} className="mt-3 p-3 bg-white rounded-lg border border-purple-200 space-y-3">
          ...
          <div>
            <label className="block text-[11px] font-semibold text-slate-700 mb-1">
              Citizen Uploaded New Evidence Photo
            </label>
            <div className="flex items-center space-x-2">
              <input
                type="text"
                ...
                className="flex-1 p-1.5 text-[11px] border border-slate-300 rounded font-mono"
              />
            </div>
          </div>
